...
from roi_data_layer.roibatchLoader_i3d import roibatchLoader
from model.utils.config import cfg, cfg_from_file, cfg_from_list, get_output_dir
from model.utils.net_utils import weights_normal_init, save_net, load_net, \
    adjust_learning_rate, save_checkpoint, clip_gradient

from roi_data_layer.spatial_transforms import Compose, Normalize, RandomHorizontalFlip, \
    RandomVerticalFlip, MultiScaleRandomCrop, ToTensor, CenterCrop
from roi_data_layer.temporal_transforms import TemporalRandomCrop, LoopPadding

# from model.rpn.c3d import c3d
from model.tdcnn.c3d import C3D, c3d_tdcnn
# from model.tdcnn.i3d import I3D, i3d_tdcnn
from model.tdcnn.i3d_meva import InceptionI3D, i3d_tdcnn
...
def parse_args():
    ...
    parser.add_argument('--net', dest='net', default='c3d', type=str, choices=['i3d','c3d', 'res18', 'res34', 'res50', 'eco'],
                        help='main network c3d, i3d, res34, res50')
    parser.add_argument('--start_epoch', dest='start_epoch', default=1, type=int,
                        help='starting epoch')
    parser.add_argument('--epochs', dest='max_epochs', default=20, type=int,
    ...
    parser.add_argument('--lr_decay_step', dest='lr_decay_step', default=10, type=int,
...
if __name__ == '__main__':
    ...
    # cfg.num_scales = 2
    # cfg.scale_step = 0.84089641525
    # cfg.scales = [cfg.initial_scale]
    # for i in range(1, cfg.num_scales):
    #     cfg.scales.append(cfg.scales[-1]*cfg.scale_step)

    cfg.INITIAL_SCALES = 1.0
    cfg.NUM_SCALES = 2
    cfg.SCALE_STEP = 0.84089641525
    cfg.SCALES = [cfg.INITIAL_SCALES]
    for i in range(1, cfg.NUM_SCALES):
        cfg.SCALES.append(cfg.SCALES[-1]*cfg.SCALE_STEP)
    ...
    train_transforms = {
        'spatial': Compose([MultiScaleRandomCrop(cfg.SCALES, cfg.SPATIAL_SIZE),
                            RandomHorizontalFlip(),
                            ToTensor(cfg.NORM_VALUE), Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]),  # i3d std=[0.5]
        # 'temporal': TemporalRandomCrop(cfg.SAMPLE_DURATION, is_test=False),
        'temporal': None,
        'target': None,  # ClassLabel()
    }

    validation_transforms = {
        'spatial': Compose([CenterCrop(cfg.SPATIAL_SIZE),
                            # MultiScaleRandomCrop(config.scales, config.spatial_size),#CenterCrop(config.spatial_size),
                            ToTensor(cfg.NORM_VALUE),
                            Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]),
        # 'temporal': TemporalRandomCrop(cfg.SAMPLE_DURATION, is_test=True),
        'temporal': None,
        'target': None,  # ClassLabel()
    }

    dataset = roibatchLoader(roidb, spatial_transform=train_transforms['spatial'],
