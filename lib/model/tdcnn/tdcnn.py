...
class _TDCNN(nn.Module):
    ...
    def forward(self, video_data, gt_twins):
        ...
        # assert False
        # print('base_feat: {}'.format(base_feat.size()))
        ...
        # base_feat: 1*512*96*7*7, gt_twins: 1*20*3
        ...
        if cfg.POOLING_MODE == 'pool':
            # base_feat: num_filters * L/8 * feat_dim * feat_dim
            pooled_feat = self.RCNN_roi_temporal_pool(base_feat, rois.view(-1,3))
            # pooledd_feat: num_rois * num_filters * POOLING_LENGTH * POOLING_WIDTH * POOLING_HEIGHT
            # print('base_feat: {}, poole_feat: {}'.format(base_feat.size(), pooled_feat.size()))
