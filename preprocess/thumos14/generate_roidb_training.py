...
WINS = [LENGTH * 1]
# WINS = [int(LENGTH * 2)]
FRAME_DIR = '/mnt/sda/diva/thumos14/data'
...
def generate_roidb(split, segment):
  ...
  # Use a sliding window to go over video frames, make sure the minimium overlap is no less than 0.7
  for vid in segment:
    if vid in video_list:
      ...
      for win in WINS:             # win = 768
        # inner of windows
        stride = int(win / LENGTH) # stride = 1
        # Outer of windows
        step = int(stride * STEP) # step = 768/4 = 192
        # Forward Direction
